...
class StoreAndSellerController extends Controller
{
    ...
    public function index()
    {
        ...
        $stores = Store::with(['branches', 'products', 'delegate', 'cityRelation', 'countryRelation'])
            ->get();
        // var_dump($stores);
        $sellers = collect();
        if (!$stores->isEmpty()) {
            // Fetch sellers based on store user_ids, ordered by latest created first
        ...
        return view('store-and-seller.index', compact('sellers'));
    }
    ...
    public function create()
    {
        ...
        return view('store-and-seller.create', compact(
            'sections',
            'sellerTypes',
            'countries'
        ));
    ...
    public function store(Request $request)
    {
        ...
        $seller->city = $request->input('city_id');
        $seller->country = $request->input('country_id');
        ...
        return redirect()->route('store-and-seller.index')->with('success', 'Store and seller have been successfully Added');
    }
    ...
    private function generateStoreQrCode(Store $store): void
    {
        ...
        $qrContent = route('store.discount', ['uuid' => $store->uuid]);
    ...
    public function edit(User $seller)
    {
        ...
        return view('store-and-seller.edit', compact('seller', 'sellerTypes', 'sections', 'countries'));
    }
    ...
    public function update(Request $request, User $seller)
    {
        ...
        try {
            ...
            return redirect()
                ->route('store-and-seller.index') // Adjust this route to your sellers listing page
