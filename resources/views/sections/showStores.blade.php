                            ...
                            <!-- <a class="btn btn-success btn-sm" href="{{route('sections.showAttachStore',$section->uuid)}}">+add store</a> -->
                                <table id="myTable" class="table small-table-text text-center">
                                    ...
                                    @foreach ($stores  as $store )
                                        ...
                                        <td class=" text-black"><b>{{ $store->name ?? 'No Name' }}</b></td>
                                            <td>{{$store->user->first_name ?? 'N/A'}} {{$store->user->last_name ?? ''}}</td>

                                            <td>
                                                @if(isset($store->description) && !empty($store->description))
                                                            @php
                                            ...
                                            <td style="color: green;">
                                               <b>
                                                {{$store->points ?? 0}}
                                                @if($loop->iteration==1 && ($store->points ?? 0) > 0) <span style="color: red;">(highest)</span>@endif
                                               </b>
                                            </td>
                                            <td>
                                        ...
                                        <td>
                                            @if($loop->iteration==1 && ($store->points ?? 0) > 0)
